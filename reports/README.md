...
- **평가 피드백 루프**: 도출된 평가 결과를 기반으로 에이전트의 거부(Refusal) 기준을 완화하거나 프롬프트를 튜닝하여 평가 점수를 지속적으로 개선합니다 (예: 평균 70점 $\\rightarrow$ 90점 상승 등).

- **평가 데이터 자동 증강(Data Augmentation)**: 정적인 뼈대 데이터를 기반으로 LLM을 활용해 `어조 변환(Tone Shift)`, `변수 치환(Variable Swap)`, `멀티턴 대화(Multi-turn)` 등 복합적인 상황을 모사한 **총 50개 규모의 고품질 평가 데이터셋(Golden QA)**을 완전 자동화 방식으로 증강하여 Supabase에 상시 운용합니다.
